...
if [[ "$full_path" == *"/riboseq/"* ]]; then
        echo "running riboseq parameters on $r1_filename"
    ...
    elif [[ "$full_path" == *"/rnaseq/"* ]]; then
        echo "running rnaseq parameters on $r1_filename"
        ...
        apptainer exec -B "/hpc:/hpc" --env LC_ALL=C.UTF-8 ${container_dir}/trimgalore-0.6.6.sif trim_galore \
          ${outdir}/trimgalore/${sample_id}/"${r1_filename%.*.*}.29bp_5prime.fq.gz" \
          --cores 2 \
          --gzip \
          --length 25 \
          --trim-n \
          ...
          mv "${outdir}/trimgalore/${sample_id}/${r1_filename%.*.*}.29bp_5prime_trimmed.fq.gz" "${outdir}/trimgalore/${sample_id}/${r1_filename}"
