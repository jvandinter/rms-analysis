...
```{r load ORFquant results}

load(paste0(savedir, "/orfs_separate.RData"))
load(paste0(savedir, "/orfs_combined.RData"))
load(paste0(savedir, "/orfs_overlap.RData"))
...
# Tissue
tis_files <- gsub("-RD_final_ORFquant_results","",basename(subset(orfquant_files,grepl("-TIS-",orfquant_files))))

# # organoid
org_files <- gsub("-SEDH_final_ORFquant_results","",basename(subset(orfquant_files,grepl("-ORG-",orfquant_files))))

```

# Create total/shared orfs graph
```{r shared ORFs count}
orf_count_data <- get_overlap_counts(orfquant_overlap,thresholds, orf_methods[1])

orf_count_data <- bind_rows(orf_count_data) %>% 
  mutate(value_1000 = value / 1000, perc = x * 100)  %>% 
  mutate(name = factor(name, levels = c("All","Viable","Half","Common")))

ggplot(orf_count_data,aes(x=perc, y=value_1000, colour=Source)) + 
  geom_vline(xintercept= 80,alpha = 0.3, linewidth = 4) +
  geom_line(aes(linetype=name)) +
  scale_y_continuous(name="Number of ORFs (x1000)", expand = c(0, 0), 
                     limits = function(y) c(0,max(y))) +
  scale_x_continuous(name = "Percentage overlap required", expand = c(0, 0)) +
...
ggsave("RMS_ORF_counts.pdf", device = "pdf", path = savedir,
       width = 1400, 
       height = 700, units = "px")
```


# Separate ORG and TIS combined ORF lists
```{r separate TIS & ORG}
...
mat_80 <- as.data.frame(mat_80) %>%
  dplyr::mutate(orf_id = names(orfquant_orfs_combined),
                count = rowSums(.[1:51]!=0))
colnames(mat_80) <- c(org_files,tis_files,"parent","orf_id","count")

tis_80_mat <- mat_80[,colnames(mat_80) %in% c("orf_id",tis_files,"parent","count")] %>%
  dplyr::mutate(count = rowSums(.[1:length(tis_files)]!=0))

org_80_mat <- mat_80[,colnames(mat_80) %in% c("orf_id",org_files,"parent","count")] %>%
  dplyr::mutate(count = rowSums(.[1:length(org_files)]!=0))

```

```{r create TIS GTF}

tis_orfs_combined <- subset(orfquant_orfs_combined, names(orfquant_orfs_combined) %in% 
                    rownames(tis_80_mat[which(tis_80_mat$count > 2),]))

tis_gtf <- tis_orfs_combined %>%
  unlist() %>%
  rtracklayer::export.gff(con = paste(savedir,"tissue_ORFs.gtf"))

```

```{r create ORG GTF}

org_orfs_combined <- subset(orfquant_orfs_combined, names(orfquant_orfs_combined) %in% 
                    rownames(org_80_mat))

org_gtf <- subset(orfquant_orfs_combined, names(orfquant_orfs_combined) %in% 
                    rownames(org_80_mat[which(org_80_mat$count > 2),])) %>%
  unlist() %>%
  rtracklayer::export.gff(con = paste(savedir,"tumoroid_ORFs.gtf"))
...
# Subset orfs by occurrence
orfs_occurence <- filter_orfs(org_80_mat,org_orfs_combined)
...
# Check RMS-specific ORFs

```{r}

orfs_combined <- subset(orfquant_orfs_combined, names(orfquant_orfs_combined) %in% 
                    rownames(mat_80[which(mat_80$count > 2),]))

found_ORFs <- gsub("[.]\\d+$","",gsub("_.*","",grep("MSTRG",names(orfs_combined),value = T))) %>%
  subset(. %in% pass_novel$gene_id)

rms_spec_orf_mat <- mat_80[which(gsub("[.]\\d+$","",
                                                gsub("_.*","",rownames(mat_80))) %in% found_ORFs),] %>%
  dplyr::filter(count > 1)

rms_spec_orf_tis <- tis_80_mat[which(gsub("[.]\\d+$","",
                                                gsub("_.*","",rownames(tis_80_mat))) %in% found_ORFs),] %>%
  dplyr::filter(count > 1)

rms_spec_orf_org <- org_80_mat[which(gsub("[.]\\d+$","",
                                                gsub("_.*","",rownames(org_80_mat))) %in% found_ORFs),] %>%
  dplyr::filter(count > 1)
...
cols <- viridis::inferno(3, alpha = 0.6, begin = 0.45, end = 0.85)
plot_df <- data.frame(count = c(rms_spec_orf_org$count,
                                rms_spec_orf_tis$count),
                      group = c(rep("org",length(rms_spec_orf_org$count)),
                                rep("tis",length(rms_spec_orf_tis$count))),
                      orf_id = c(rms_spec_orf_org$orf_id,
                                rms_spec_orf_tis$orf_id)
  
)
ggplot(plot_df,aes(count, fill = group)) +
...
```{r upset novel ORFs}
orf_list <- list(`RMS-spec genes` = pass_novel$gene_id,
                 `found RMS ORFs` = gsub("[.]\\d+$","",gsub("_.*","",grep("MSTRG",names(orfs_combined),value = T))),
                 `RMS-spec tissue ORF` = gsub("[.]\\d+$","",
                                                gsub("_.*","",rownames(rms_spec_orf_tis))),
                 `RMS-spec organoid ORF` = gsub("[.]\\d+$","",
                                                gsub("_.*","",rownames(rms_spec_orf_org)))
                 )


m = ComplexHeatmap::make_comb_mat(orf_list)


pdf(file = paste(savedir,"RMS_upset_ORF_novel.pdf", sep = "/"),
    width = 6,
    height = 4)
draw(ComplexHeatmap::UpSet(m,
                      pt_size = unit(5, "mm"), 
                      column_title = "RMS novel genes with ORFs",
                      lwd = 3,
                      comb_col = c("red", "blue","forestgreen", "black")[comb_degree(m)],
...
**Final remark error:**
Had to use RMS GTF & Rannot instead of ensembl v102 reference GTF
Added MSTRG regex to the functions of add_stop_codon & code for generating the reference

```{r troubleshooting}

orfs <- prepare_orfquant(orfquant_files[1:3])

# Gives error:
test <- add_stop_codon(orf_granges = orfs[[1]],gtf_exon = gtf_exon)


# error in add_stop_codon, see what happens
