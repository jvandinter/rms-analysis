...
wd = "/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/analysis"

savedir = "/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/results"

multiqc_name = "ribo_rms"
...
trim_plot <- trim_stats %>%
  ...
  dplyr::mutate(Sample = gsub("-1-AL.*","",Sample))
...
for (i in seq_along(contaminant_files)){
  ...
  contaminants$Sample <- paste(
    strsplit(basename(contaminant_files[i]), "-AL_")[[1]][1], 
    collapse = "-")
...
frame_perc_plot <- tidyr::pivot_longer(perc_reads_df, cols = colnames(perc_reads_df))
...
ggplot(frame_perc_plot, aes(y = value, x = name)) +
...
ggplot(data = inframe_df, aes(y = mean_pct_max_frame, x = reorder(rownames(inframe_df), -mean_pct_max_frame))) +
...
ggplot(data = read_cats_annotgenes_perc,
