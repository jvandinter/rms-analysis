...
scriptdir="/hpc/pmc_vanheesch/projects/jvandinter/rms_analysis/02_riboseq/scripts/scripts_clean/orf_quantification"
riboseqc_parent_dir="${outdir}/RiboseQC"

# Set input variables
orf_gtf="/hpc/pmc_vanheesch/projects/jvandinter/rms_analysis/02_riboseq/analysis/price/RMS_merged_price_reduced_ORFs.gtf"  # GTF file containing ORF definitions (eg CDS regions)
is_anno="no"  # Is orf_GTF an annotation file with incomplete ORFs? (i.e. from Ensembl) [yes/no]
out_dir="/hpc/pmc_vanheesch/projects/jvandinter/rms_analysis/02_riboseq/analysis/price_p_site_quantification"  # Directory to output generated files
mkdir -p ${out_dir}
riboseqc_dir="${outdir}/RiboseQC/"  # Parent directory containing RiboseQC results of all samples
analysis_name="RMS_price_merged_quant"  # Name of analysis run
id_type="ORF_id"
