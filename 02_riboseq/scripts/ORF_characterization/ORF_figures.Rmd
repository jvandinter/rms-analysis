...
# Tissue
orfquant_files <- subset(orfquant_files,grepl("-TIS-",orfquant_files))

# # organoid
# orfquant_files <- subset(orfquant_files,grepl("-ORG-",orfquant_files))
...
/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/results
...
orfs_half <- orfquant_orfs_combined[orfs_occurence %>% 
                                      filter(rowsum >= 10) %>% rownames()]

orfs_common <- orfquant_orfs_combined[orfs_occurence %>% 
                                      filter(rowsum >= 19) %>% rownames()]
```
