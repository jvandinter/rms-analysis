...
orf_combined = "/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/results/patient/orfs_combined.RData"
orf_overlap = "/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/results/patient/orfs_overlap.RData"
orf_separate = "/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/results/patient/orfs_separate.RData"
sample_ids <- gsub("-.*","",grep("TIS",basename(list.dirs("/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/analysis/ORFquant", recursive = F)), value = T))
...
write.table(interesting_orfs, file = paste(savedir,"patient","RMS_patient_all_ORFs.txt",sep="/"), quote = F, row.names = F, sep =";")
...
```{r Parse ORF data}
interesting_orfs <- read.delim(paste(savedir,"patient","RMS_patient_novel_ORF.txt",sep="/"), sep =";")
...
```{r tumoroid FASTA}
fasta_locs = grep("TIS",list.dirs("/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/analysis/ORFquant", 
                                  recursive = F), value = T)
savedir = "/hpc/pmc_vanheesch/projects/Jip/rms_analysis/02_riboseq/results"
filtered_interesting_orfs = read.delim(paste(savedir,"patient","RMS_patient_all_ORFs.txt",sep="/"), sep =";")
...
Biostrings::writeXStringSet(patient_fa, filepath = paste(savedir,"patient","patient_ORFs.fa", sep = "/"),
                            format = "fasta")
...
tis_seq_table <- read.delim(file = paste(savedir,"patient/patient_sequence_table.txt", sep = "/")) %>%
  dplyr::filter(width > 18 & width < 150)

patient_fa <- Biostrings::AAStringSet(tis_seq_table[,c("sequence")])
names(patient_fa) <- tis_seq_table[,c("names")]

Biostrings::writeXStringSet(patient_fa, filepath = "/hpc/pmc_vanheesch/projects/Jip/rms_analysis/03_prediction/data/patient_sORF.fa",
                            format = "fasta")
