...
tum_seq_table <- read.delim(file = paste(savedir,"tumoroid/tumoroid_sequence_table.txt", sep = "/"), sep = " ") %>%
  dplyr::filter(width > 18 & width < 201)

tumoroid_fa <- Biostrings::AAStringSet(tum_seq_table[,c("sequence")])
names(tumoroid_fa) <- tum_seq_table[,c("names")]

Biostrings::writeXStringSet(tumoroid_fa, filepath = "/hpc/pmc_vanheesch/projects/Jip/rms_analysis/03_prediction/data/tumoroid_sORF.fa",
                            format = "fasta")
