...
write.table(interesting_orfs, file = paste(savedir,"patient","RMS_patient_novel_ORF.txt",sep="/"), quote = F, row.names = F, sep =";")
...
```{r plot }
single_orf %>% dplyr::mutate(orf_type = factor(orf_type, levels = c("uORF","novel","lncORF","dORF"))) %>%
...
single_orf %>% dplyr::filter(pass == T)  %>%
  dplyr::mutate(orf_type = factor(orf_type, levels = c("uORF","novel","dORF"))) %>%
  ggplot(aes(x = count, fill = orf_type)) +
  geom_histogram(stat = "count") +
  scale_x_continuous(breaks = 2:19) +
  facet_wrap(~ orf_type, scales = "free_y") +
