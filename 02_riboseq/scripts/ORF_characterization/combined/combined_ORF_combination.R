...
combined_file <- paste0(savedir, "/combined/orfs_combined.RData")
separate_file <- paste0(savedir, "/combined/orfs_separate.RData")
overlap_file <- paste0(savedir, "/combined/orfs_overlap.RData")
...
orfquant_files = list.files(paste(wd,"analysis","ORFquant", sep = "/"),
                            pattern = ".*RMS.*final_ORFquant_results",
                            recursive = T,
                            full.names = T)
