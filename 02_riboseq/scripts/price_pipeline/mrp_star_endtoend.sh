...
if [[ -s "${outdir}/star_end2end/${sample_id}/${sample_id}.Aligned.sortedByCoord.out.bam.bai" ]]; then
...
# Create output dirs
cd "${outdir}"
mkdir -p "star_end2end/${sample_id}/"
...
apptainer exec -B /hpc:/hpc ${container_dir}/STAR-2.7.8a.sif STAR \
  ...
  --outFileNamePrefix "${outdir}/star_end2end/${sample_id}/${sample_id}." \
...
apptainer exec -B /hpc:/hpc ${container_dir}/samtools-1.12.sif samtools index \
  -@ ${threads} \
  "${outdir}/star_end2end/${sample_id}/${sample_id}.Aligned.sortedByCoord.out.bam"
