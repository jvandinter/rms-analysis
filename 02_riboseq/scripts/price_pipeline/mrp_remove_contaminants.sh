...
if [[ -s "${outdir}/bowtie2/${sample_id}/${sample_id}_filtered.fastq.gz" ]]; then
  echo "`date` ${sample_id} filtered FASTQ already present"
  exit 0
fi
...
apptainer exec -B /hpc:/hpc,${TMPDIR}:${TMPDIR} ${container_dir}/bowtie2-2.4.2.sif bowtie2 \
  --seedlen=25 \
  --threads ${threads} \
  --time \
...
for contaminant_type in tRNA snRNA snoRNA mtDNA rRNA ; do  
  contaminant_reads=`apptainer exec -B /hpc:/hpc,${TMPDIR}:${TMPDIR} ${container_dir}/samtools-1.12.sif samtools view -@ ${threads} "${TMPDIR}/${sample_id}_contaminants.sam" | grep -o "$contaminant_type" | wc -l`
