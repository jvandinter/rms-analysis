...
mkdir -p ${project_folder}/log/price_${run_id}/{star_end2end,convert} 
...
echo -e "\n`date` Convert BAM to FASTQ ..."
echo -e "====================================================================================== \n"

convert_jobid=()

convert_jobid+=($(sbatch --parsable \
  --mem=8G \
  --cpus-per-task=4 \
  --time=24:00:00 \
  --array 1-${#sample_ids[@]}%${simul_array_runs} \
  --job-name=${run_id}.convert \
  --output=${project_folder}/log/price_${run_id}/convert/%A_%a.out \
  --export=ALL \
  ${scriptdir}/mrp_convert.sh 
))

if [[ ${#convert_jobid[@]} -eq 0 ]]; then
  fatal " job not submitted successfully, convert_jobid array is empty"
fi

info "trimgalore jobid: ${convert_jobid}"
...
star_jobid+=($(sbatch --parsable \
  ...
  --output=${project_folder}/log/price_${run_id}/star_end2end/%A_%a.out \
  --dependency=aftercorr:${convert_jobid} \
  --export=ALL \
