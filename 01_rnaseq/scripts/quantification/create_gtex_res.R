...
txi <-
  readRDS(paste(savedir, "processed", "RMS_counts_full.RDS", sep = "/"))
...
txi_gtex <-
  list(
    abundance = txi$abundance[, which(colnames(txi$abundance) %in% c(samples_gtex))],
    counts = txi$counts[, which(colnames(txi$counts) %in% c(samples_gtex))],
    length = txi$length[, which(colnames(txi$length) %in% c(samples_gtex))],
...
meta_gtex$condition <-
  factor(meta_gtex$condition, levels = c("healthy", tumor_type))
...
saveRDS(gtex_dds,
        file = paste(savedir, "results", "GTEX_DESEQ_type.RDS", sep = "/"))
...
for (i in 2:length(levels(meta_gtex$type))) {
  ...
  write.table(res, file = paste(savedir, "results", paste0(
    "GTEX_res_", gsub(" ", "_", levels(meta_gtex$type)[i]), ".txt"
  ), sep = "/"))
}
