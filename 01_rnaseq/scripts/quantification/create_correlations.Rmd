...
fusion_genes <- unique(gtf_df[which(gtf_df$gene_name %in% c("PAX3","PAX7","FOXO1")),]$gene_id)
...
fp_counts <- fp_counts[which(rownames(fp_counts) %in% c(picked_genes,fusion_genes)),]
...
annot_col <- data.frame(gene_id = rownames(fp_counts)) %>%
  ...
  dplyr::distinct() %>%
  dplyr::mutate(fusion = ifelse(gene_id %in% fusion_genes,"A","B"),
                gene_biotype = ifelse(is.na(gene_biotype),"stringtie",gene_biotype),
...
anno_cols <- list(gene_biotype = setNames(viridis::rocket(n = 4),unique(annot_col$gene_biotype)),
                  ...
                  fusion = setNames(viridis::plasma(n = 2),unique(annot_col$fusion))
                  )
