...
```{r}
fp_pass_novel <- read.delim(paste(savedir_base,"FP-patient",
                                  paste0("RMS_FP-patient_novel_enriched_matrix.txt"),sep = "/"), 
                            sep = ";", header = T)
fp_pass_canon <- read.delim(paste(savedir_base,"FP-patient",
                                  paste0("RMS_FP-patient_canon_enriched_matrix.txt"),sep = "/"), 
                            sep = ";", header = T)
fn_pass_novel <- read.delim(paste(savedir_base,"FN-patient",
                                  paste0("RMS_FN-patient_novel_enriched_matrix.txt"),sep = "/"), 
                            sep = ";", header = T)
fn_pass_canon <- read.delim(paste(savedir_base,"FN-patient",
                                  paste0("RMS_FN-patient_canon_enriched_matrix.txt"),sep = "/"), 
                            sep = ";", header = T)

combined_pass_novel <- read.delim(paste(savedir_base,"old",
                                  paste0("RMS_novel_enriched_matrix.txt"),sep = "/"), 
                            sep = ";", header = T)

combined_pass_canon <- read.delim(paste(savedir_base,"old",
                                  paste0("RMS_canon_enriched_matrix.txt"),sep = "/"), 
                            sep = ";", header = T)

venn_canon <- list(`FP-RMS` = fp_pass_canon[which(fp_pass_canon$pass == T),]$gene_id,
                   `FN-RMS` = fn_pass_canon[which(fn_pass_canon$pass == T),]$gene_id,
                   `combined RMS` = combined_pass_canon[which(combined_pass_canon$pass == T),]$gene_id)
...
For example, #4FB3A3 could be a nice choice.

If you prefer a contrasting color, you might consider a light or muted shade of yellow or orange. For instance,  or #FFB07C could complement the green and red tones in your original colors.
