...
dds_t <- readRDS("/hpc/pmc_vanheesch/projects/jvandinter/custom_transcriptomes/20221020_JD_quant_tumor_cohorts/analysis/RMS/results/RMS_DESEQ_tumoroid_subtype.RDS")

meta_t <- read.delim(paste(basedir,"documentation",
                            "tumoroids_rna_ids.txt",
                            sep = "/")) %>%
  dplyr::distinct()

vsd_t <- DESeq2::varianceStabilizingTransformation(dds_t)
plot <- plotPCA(vsd_t, intgroup = c("condition","sample_id"), returnData = T) %>%
  dplyr::rename(sequence_id = sample_id) %>%
  dplyr::left_join(meta, by = "sequence_id") %>%
  dplyr::mutate(tumoroid = gsub("-.","",sample_id) )

ggplot(plot, aes(x = PC1,y = PC2, col = tumoroid)) + geom_point()
