...
```{r parameters}
basedir = "/hpc/pmc_vanheesch/projects/jvandinter/rms_analysis/01_rnaseq"
savedir = paste(basedir,"results/quantification/figures",sep="/")
metadata_dir = paste(basedir,"documentation",sep="/")
txome_gtf = "/hpc/pmc_vanheesch/projects/jvandinter/rms_analysis/01_rnaseq/analysis/rnaseq_pipeline/customannotation/RMS_full_novel_filtered_corrected.sorted.gtf"
tumor_type = "RMS"
fp_target_genes = read.delim(file = paste("/hpc/pmc_vanheesch/projects/jvandinter/rms_analysis/", "documentation","FP_target_genes.txt", sep = "/"), header = T)
...
```{r load & prepare data}
txi <- readRDS(paste(basedir, "RMS_counts_full.RDS", sep = "/"))
...
Actual analysis:
```{r read data}
tumor_arms <- read.delim(paste(basedir,"documentation","RMS_fp.txt",sep = "/"), sep = ";")
tumor_dds <- readRDS(paste(basedir,"analysis","quantification","deseq2",paste0(tumor_type,"_patient_condition.RDS"), sep = "/"))
...
healthy_cohort = rbind(r2_cohort,gtex_cohort,evo_cohort)

write.table(healthy_cohort, file = paste("/hpc/pmc_vanheesch/projects/jvandinter/rms_analysis/documentation/healthy_cohort.txt"), quote=F,sep =";")
